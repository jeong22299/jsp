...
<%
	...
	// 1) P1234에 해당되는 productVO를 가져옴
	// select * from product where product_id = 'P1234'
	// id : P1234
	ProductVO product = dao.getProductById(id);
	if(product == null){	// 해당 상품이 없으면 예외처리 페이지로 이동
		response.sendRedirect("exceptinNoProductId.jsp;");
	}
	...
	if(list == null){
		list = new ArrayList<ProductVO>();   // list는 null이므로 여기서 리스트를 생성해주자
		session.setAttribute("cartlist", list);  // cartlist라는 세션 속성 명으로 세션 생성
	}
	
	int cnt = 0;	// 장바구니에 있는 상품이 파라미터로 넘어온 상품코드(id)와 같으면 1 증가
	// 3-1) cartlist라는 장바구니에 P1234라는 상품이 이미 있는 경우
	// 		P1234라는 상품 객체(productVO)의 quantity 멤버변수의 값을 1 증가
	...
	// 3-2) cartlist라는 장바구니에 P1234라는 상품이 없는 경우(cnt => 0)
	//		cartlist라는 장바구니에 P1234라는 상품을 넣자. quantity 값을 1로 처리
	if(cnt==0){
...
// 	}
	...
	// 4) product.jsp?id=P1234 경로로 되돌아가자
