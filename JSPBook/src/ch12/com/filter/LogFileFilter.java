...
public class LogFileFilter implements Filter {
	 ...
	 */
	// fileConfig : {"filename" : "c:\\logs\\monitor.log"}
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		try {
			...
			//					자동 flush 기능 탑재
			//					생성자에 FileNotFoundException 예외가 있어, 반드시 예외처리를 해야함
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		// 날짜 형식 지정 객체 생성
		DateFormat fomatter = new SimpleDateFormat("yyy/MM/dd HH:mm:ss");
		Calendar calendar = Calendar.getInstance();
		calendar.setTimeInMillis(System.currentTimeMillis());
		
		// monitor.log 파일에 입력될 데이터 생성
		writer.printf("현재 일시 : %s %n", fomatter.format(calendar.getTime()));
		// 사용자 IP 주소
